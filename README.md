...
脚本会在知乎创作中心推荐问题列表、知乎问题页和回答详情页中，自动显示问题的「缺口值」和「答题分」。

问题页和回答详情页的小框固定悬浮在右上角、头像下方，页面向下滚动时仍保持可见。页顶优先紧贴“关注者 / 被浏览”统计区右侧，右侧空间较窄时分行显示；没有足够侧边空间时紧贴统计数字下方。未找到头像或统计区时使用右上角的默认位置。

悬浮小框分行显示时，统一使用“名称：内容”：
...
小框底色和边框随整体机会等级变化：极高用浅紫、高用浅绿、中用浅琥珀、低用浅灰；标签为灰色，数值为深色。等级文字分别使用紫色（极高）、绿色（高）、琥珀色（中）、灰色（低）；缺口等级与答题分等级独立着色。没有回答时，缺口值显示“无回答”，不标注缺口等级。

“等你来答”页面（`/question/waiting`）中的标签保持单行，沿用上述分级配色；窄窗口可在标签区域横向滚动查看完整内容。

```text
机会等级：高｜缺口值：2,149（中）｜答题分：85（高）
```
...
```text
等级：高质｜质效分：50｜赞同率：1.23%｜收藏率：2.06%
```

标签保持单行，底色、边框和等级文字随质效等级变化：极优用紫色、高质用绿色、中等用琥珀色、待观察用灰色。字段名称使用灰色，数值使用深色；赞同率与收藏率不另设等级。窄窗口下可在标签区域横向滚动查看完整内容。
